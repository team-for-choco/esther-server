...
object ItemPriceRegistry {
    ...
    private fun registerCustomItemPrices() {
        ...
        register(Mod.FISH_STEW, 55, ShopCategory.FOOD)          // 🐟 붕어 (36→55)
        register(Mod.GIMBAP, 60, ShopCategory.FOOD)             // 🐟 멸치 (44→60)
        register(Mod.KIMCHI, 60, ShopCategory.FOOD)
        register(Mod.KIMCHI_STEW, 125, ShopCategory.FOOD)        // 2차 가공 프리미엄 (69→125)
        register(Mod.MISO_SOUP, 45, ShopCategory.FOOD)
        register(Mod.GRILLED_MACKEREL, 58, ShopCategory.FOOD)   // 🐟 고등어 (40→58)
        register(Mod.EGG_RICE, 49, ShopCategory.FOOD)
        // Cooking dishes - Advanced
        register(Mod.SASHIMI_PLATTER, 140, ShopCategory.FOOD)   // 🐟🐟 도미+연어 (85→140)
        register(Mod.EEL_RICE, 120, ShopCategory.FOOD)          // 🐟 장어 (95→120)
        ...
        register(Mod.SEAFOOD_PANCAKE, 105, ShopCategory.FOOD)   // 🐟🐟 오징어+새우 (80→105)
        ...
        register(Mod.BLOWFISH_SASHIMI, 210, ShopCategory.FOOD)  // 🐟 복어(희귀) (160→210)
