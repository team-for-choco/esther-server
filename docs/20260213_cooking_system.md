...
## 인게임 테스트
- [x] 조리대 크리에이티브 모드에서 획득
- [x] 조리대 설치 → 서바이벌에서 파괴 불가 확인
- [x] 재료 순서대로 우클릭 → 파티클/소리 피드백
- [x] 빈 손 우클릭 → 요리 완성 (정확한 레시피)
- [x] 틀린 조합 → 재료 소멸 + 실패 이펙트
- [x] 시금치비빔밥: 밥 → 시금치 → 고추 → 빈 손 (영양 10, 포만 0.8)
- [x] 생선찌개: 구운 테스트 생선 → 고추 → 시금치 → 빈 손 (영양 10, 포만 0.8)
- [x] 김밥: 밥 → 구운 테스트 생선 → 시금치 → 빈 손 (영양 12, 포만 0.9)
- [x] 수확물비빔밥: 밥 → 구운 테스트 수확물 → 고추 → 시금치 → 빈 손 (영양 14, 포만 1.0)
- [x] 요리 섭취 → 영양/포만감 확인 (정확한 수치는 코드 기반 검증)
- [x] 희귀 재료로 제작 → 높은 등급 확률 확인 (확률 로직은 코드 기반 검증)
