...
### Phase B: 마일스톤 + 칭호
- [x] 아이템 1종 등록 → "첫 발견" 마일스톤 서버 전체 공지
- [x] `/title list` → 달성/미달성 목록 출력 (진행률 포함)
- [x] `/title select first_discovery` → 채팅에 [초보 수집가] 표시
- [x] `/title clear` → 칭호 해제
- [x] G키 → 도감 GUI → 칭호 탭 표시
- [x] 칭호 탭에서 달성 마일스톤 클릭 → 칭호 선택
- [x] 칭호 탭에서 활성 칭호 재클릭 → 칭호 해제
- [x] 미달성 마일스톤 클릭 → 반응 없음
- [ ] 탭 목록(TAB)에 칭호 표시 확인 (싱글 환경 한계로 멀티 테스트 필요)
- [x] 사망 후/재접속 → 마일스톤 + 칭호 유지
