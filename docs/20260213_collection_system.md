...
| `OpenCollectionPayload` | C→S | G키 → 서버에 데이터 요청 |
...
### 도감대 블록
- [ ] 도감대 블록이 크리에이티브 탭에 표시됨
- [ ] 도감대 설치 가능
- [ ] 도감대 파괴 불가 (서바이벌)

### 아이템 등록
- [ ] collectible 아이템 들고 도감대 우클릭 → 등록 메시지 + 아이템 소멸
- [ ] 등록 불가 아이템 (예: 돌) 들고 우클릭 → "등록할 수 없습니다" 메시지, 소멸 안 함
- [ ] 이미 등록된 아이템 재등록 시도 → "이미 등록" 메시지, 소멸 안 함
- [ ] 품질 있는 아이템 (일반/고급/희귀) 각각 별도 등록 확인
- [ ] 품질 없는 아이템 (씨앗류) 등록 확인
- [ ] 품질 태그 대상인데 품질 없는 아이템 → "등급이 없는 아이템" 메시지
- [ ] 등록 시 사운드 재생
...
- [ ] 등록한 아이템이 아이콘으로 표시됨
- [ ] 미등록 아이템이 "?"로 표시됨
- [ ] 카테고리 탭 전환 (전체/생선/농산물/광물/요리)
- [ ] 등록한 아이템에 마우스 호버 → 이름 + 등급 툴팁
- [ ] 미등록 아이템에 마우스 호버 → "???" 툴팁
- [ ] 진행률 바 + 텍스트 정상 표시
- [ ] ESC로 GUI 닫기
- [ ] GUI 열린 상태에서 게임 일시정지 안 됨

### 데이터 유지
- [ ] 사망 후 리스폰 → 도감 데이터 유지 (G키로 확인)
- [ ] 로그아웃 후 재접속 → 도감 데이터 유지
- [ ] 차원 이동 (네더/엔드) → 도감 데이터 유지
