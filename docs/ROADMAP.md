...
- [ ] **1-1. 루트 모디파이어 replace 방식 구현**
  - 테스트 생선이 기존 생선을 대체하도록 변경 (add → replace)
  - `doApply`에서 `generatedLoot.clear()` 후 `add()` 또는 별도 `ReplaceItemLootModifier` 생성
  - 참고: [20260207_fix_loot_modifier.md](20260207_fix_loot_modifier.md)
- [ ] **1-2. 커스텀 광물 + 월드젠** ★★★
  - 광석 블록 + 드롭 아이템 + 월드젠 (Y좌표, 생성 빈도, 바이옴)
  - 제련 레시피
  - 참고: 띵타이쿤 (커스텀 광물 3종 + 보석 3종)
- [ ] **1-3. 아이템 등급/품질 시스템** ★★★
  - 공통 등급 프레임워크 설계 (일반/은별/금별 또는 일반/희귀/전설)
  - 기존 생선/작물/광물에 등급 적용
  - 이후 모든 컨텐츠가 이 위에 구축됨
  - 참고: 악어놀이터 (은별/금별), 초기 기획 (일반→희귀→특별)
- [ ] **1-4. 한국식 작물 추가** ★★☆
  - 쌀, 고추, 시금치 등 한국 농촌 감성 작물
  - 기존 작물 패턴 재활용
  - 참고: 별농서버
