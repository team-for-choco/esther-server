...
### 1. 아이템 확인
- [X] 크리에이티브 탭 "Esther Server"에 테스트 씨앗, 테스트 수확물, 구운 테스트 수확물 표시
- [X] 각 아이템 텍스처 정상 렌더링 (씨앗=밀씨앗, 수확물=밀, 조리=빵)
- [X] 한국어 이름 정상 표시 (영어는 미확인이나 코드상 정상)

### 2. 작물 심기 & 성장
- [X] 경작지(Farmland)에 테스트 씨앗 우클릭으로 심기
- [X] 심은 후 작물 블록 정상 렌더링
- [X] 뼛가루(Bone Meal)로 성장 촉진
- [X] 8단계(age 0~7) 텍스처 변화 확인

### 3. 수확
- [X] 완전 성장(age=7) 파괴 시 테스트 수확물 + 테스트 씨앗 드롭
- [X] 미성장 파괴 시 테스트 씨앗 드롭 확인 (⚠️ 테스트 생선도 함께 드롭되는 현상은 CHOCO-74의 글로벌 루트 모디파이어 버그 - GitHub Issue #3으로 분리)

### 4. 조리
- [X] 화로: 테스트 수확물 → 구운 테스트 수확물 (200틱)
- [X] 훈연기: 테스트 수확물 → 구운 테스트 수확물 (100틱)
- [X] 캠프파이어: 테스트 수확물 → 구운 테스트 수확물 (600틱)

### 5. 음식
- [X] 테스트 수확물 먹기 가능 (허기 2, 포만감 0.3) - 먹기 확인, 수치는 코드 하드코딩으로 보장
- [X] 구운 테스트 수확물 먹기 가능 (허기 6, 포만감 0.6) - 먹기 확인, 수치는 코드 하드코딩으로 보장

### 테스트 결과
- **전체 결과**: ✅ 통과 (CHOCO-75 범위 내 모든 기능 정상)
- **알려진 이슈**: 테스트 생선 드롭 버그는 CHOCO-74의 글로벌 루트 모디파이어 문제 (GitHub Issue #3)
